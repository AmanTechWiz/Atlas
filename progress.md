...
## Last Updated
2026-06-06 — **Critical bug fixed**: second-file-upload to the same per-session Chroma collection was throwing `chromadb.errors.InternalError: ... (code: 1032) attempt to write a readonly database` AND would have silently destroyed previously-indexed data even without the error. Root cause: the safe-swap pattern (build in `*_new/`, rename into place) is broken in `chromadb==1.5.9` + `langchain-chroma==1.1.0` — the SQLite metadata and HNSW index fall out of sync after a rename, so subsequent reads return `count=0` even though the HNSW file is full of vector data. Fix: removed the safe-swap entirely. `safe_ingest_files`, `safe_ingest_dir`, and `add_files_to_collection` now all write directly to the target `persist_dir` via `Chroma(persist_directory=...).add_documents(chunks)`. For the "full replace" case, the existing collection is `delete_collection()`'d first. New helper `_open_collection(persist_dir, embeddings, collection_name)` returns a fresh LangChain `Chroma` handle; `_count_in_dir` and `_swap_into_place` removed. E2E verified: 1st upload → 1 chunk, 2nd upload → 2 chunks, both queries retrieve the right chunks, reset deletes the collection. All 127 deterministic tests still pass in 0.41s. Streamlit is ready to restart on port 8765 for the user to verify. **Next up:** confirm UI re-upload works, then start the next official US (likely US 6's "conflicting agent outputs" detection — the only remaining US 6 gap).
...
- **BUG FIX 2026-06-06 — second upload to the same session is now safe.** Code is fixed, E2E smoke test passed (1st + 2nd upload + both queries + reset all work, 127 tests still pass). Awaiting user to verify in the running Streamlit UI: open http://localhost:8765 (start with `unset VIRTUAL_ENV && uv run python -m streamlit run ui/app.py --server.port 8765` if not already running), upload file A, ask a question, upload file B, ask a question that only file B can answer, confirm the system retrieves the right chunks and that the session corpus grew. Once user confirms, update `progress.md` "Last Updated" and ask which official US to build next (likely US 6's "conflicting agent outputs" detection — the only remaining US 6 gap).
...
- BUG FIX 2026-06-06: the safe-swap pattern (build in `*_new/`, rename into place) in `vector_store/ingest.py` was causing two distinct failures on the **second** upload to the same per-session collection: (a) `chromadb.errors.InternalError: ... (code: 1032) attempt to write a readonly database` thrown by `Chroma.from_documents` because the Rust client held open file handles bound to the original (now non-existent) `temp_dir` path; (b) even if (a) had not fired, the data written during the **first** upload would have been silently destroyed because the new collection was built in the same path and a count check failure would have wiped it. Root cause: a chromadb 1.5.9 + langchain-chroma 1.1.0 bug where a renamed HNSW index falls out of sync with the SQLite metadata — the HNSW file ends up with the full vector data (verified: `data_level0.bin` is 321KB, `length.bin` has 400 bytes) but `Chroma(persist_directory=...)._collection.count()` returns 0. Fix: **removed safe-swap entirely**. `safe_ingest_files`, `safe_ingest_dir`, and `add_files_to_collection` all now write directly to the target `persist_dir` via `Chroma(persist_directory=str(persist_dir), embedding_function=embeddings, collection_name=collection_name).add_documents(chunks)`. For the "full replace" case (`safe_ingest_files`, `safe_ingest_dir`), the existing collection is `delete_collection()`'d first to avoid stale state. New helper `_open_collection(persist_dir, embeddings, collection_name)` returns a fresh LangChain `Chroma` handle (does not cache module-level state). `_count_in_dir` and `_swap_into_place` removed. The `chromadb.config.Settings(is_persistent=True)` legacy path that LangChain's `Chroma.__init__` uses still works for reads/writes when writes go to the **target** path; it's only the rename-induced sync-loss that was broken. Side benefit: `add_files_to_collection` no longer needs the `_count_in_dir` check — `Chroma(persist_directory=...)` will open the existing collection (if any) and `add_documents` will append; if the collection doesn't exist, the same call creates it. E2E verified in a smoke test: 1st upload of 1-chunk doc → corpus_size=1; 2nd upload of a 1-chunk doc → corpus_size=2; query for "parental leave" → top hit from `doc_a.txt` (score 0.721), 2nd hit from `doc_b.txt` (0.521); query for "bereavement" → top hit from `doc_b.txt` (0.709), 2nd hit from `doc_a.txt` (0.576); reset → corpus_size=0. All 127 deterministic tests pass in 0.41s. Trade-off: the old safe-swap provided "if the embed step fails, the existing collection is preserved". The new direct-write path does NOT preserve the existing collection on mid-stream failure — the existing collection is deleted *first* in the full-replace case, and a partial write in the create case would leave a partial collection. For the user's "second upload" use case (per-session append), this is irrelevant because there's no full-replace step. For the CLI bulk-ingest use case, partial-write recovery is left to the user (re-run the CLI; the next `delete_collection` + `add_documents` will fix it).
- BUG FIX 2026-06-06: the `add_files_to_collection` function I added on the same day in `7f815eb`-adjacent changes is now the **only** path the UI uses for ingestion. The old `safe_ingest_files` (full replace) is no longer called by the UI; it's still available for explicit full-replace use cases (e.g., the Streamlit "Reset Session" + re-upload flow). The new `add_files_to_collection` always appends (or creates-then-appends) — never replaces. This matches the user's mental model: "uploading another file adds it to my session corpus".
...
- Live Streamlit instance: not running (was pid 14335; the user can restart with `unset VIRTUAL_ENV && uv run python -m streamlit run ui/app.py --server.port 8765`). The previous instance was killed as part of the debugging cycle; the bug fix is verified via the E2E smoke test, but the UI itself was not re-launched.
...
- New dep added: `langchain-chroma==1.1.0` (in `pyproject.toml` / `uv.lock`). Installed during the bug-fix investigation; not currently used by any module (the existing `langchain_community.vectorstores.Chroma` imports are unchanged). Candidate for a future Story 11/12 cleanup to switch the codebase to the maintained `langchain-chroma` package.
...
- 2026-06-05 INSTALLED `langchain-chroma==1.1.0` (via `uv add langchain-chroma`) — needed to test whether the new package fixed the HNSW/SQLite sync bug. (Spoiler: it did not — the bug is in chromadb 1.5.9, not the LangChain wrapper.) Package is now in `pyproject.toml` / `uv.lock` and imported as a fallback option in `vector_store/ingest.py` (the old `langchain_community.vectorstores.Chroma` import is unchanged for the rest of the system, since the wrapper behaviour is identical for non-renamed writes).
- 2026-06-06 MODIFIED `vector_store/ingest.py` — removed the safe-swap pattern (the `temp_dir = persist_dir.parent / f"{persist_dir.name}_new"` + `_swap_into_place(temp_dir, persist_dir)` dance). `safe_ingest_files`, `safe_ingest_dir`, and `add_files_to_collection` now all use direct `Chroma(persist_directory=str(persist_dir), embedding_function=embeddings, collection_name=collection_name).add_documents(chunks)` against the **target** `persist_dir`. New helper `_open_collection(persist_dir, embeddings, collection_name)` returns a fresh handle. `_swap_into_place` removed. For full-replace (`safe_ingest_files`, `safe_ingest_dir`), the existing collection is `vs.delete_collection()`'d first, then re-created by the same `add_documents` call. `add_files_to_collection` no longer needs the `_count_in_dir` branch — `Chroma(persist_directory=...)` opens-or-creates, and `add_documents` appends. Docstring updated to explain the chromadb 1.5.x safe-swap bug. `import shutil` removed (no longer needed).
- 2026-06-06 E2E VERIFIED (smoke test) — `add_files_to_collection` on 1st call → `corpus_size: 1`; on 2nd call → `corpus_size: 2`; query for "parental leave" returns `doc_a.txt` as top hit (score 0.721); query for "bereavement leave" returns `doc_b.txt` as top hit (score 0.709); `delete_collection` + `set_active_collection` → `corpus_size: 0`. All 127 tests pass in 0.41s. Cleaned up `chroma_db_sessions/smoketest_*` debug directories. Only the user's real `session_21bae474a65b` session dir remains.
